...
bool HTTPRequest::parseRequestLine(const std::string &line, const Config &config)
{
    ...
    if (!(iss >> method >> path >> http_version))
        return (print_message("Invalid request: " + line, RED), false);
    trim(method);
    trim(path);
    trim(http_version);
    if (method != "GET" && method != "POST" && method != "DELETE")
        return (print_message("Invalid request method: " + method, RED), false);
    if (http_version != "HTTP/1.1")
        return (print_message("Invalid HTTP version: " + http_version, RED), false);
    ...
    if (path.empty() || path[0] != '/')
        return (print_message("Invalid path: " + path, RED), false);
    if (path == "/favicon.ico")
        return true;
    if (path[0] == '/')
        path = path.substr(1);
    ...
    if (new_path.empty())
    {
        ...
        if (new_path.empty())
            return (print_message("No default root specified", RED), false);
    }
    ...
    if (isValidPath(new_path, false))
    {
        ...
        if (location.getPath().empty())
            return (print_message("Path not found in locations: " + dir, RED), false);
        path = new_path;
    ...
    else if (isValidPath(new_path, true))
    {
        ...
        if (location.getPath().empty())
            return (print_message("Path not found in locations: " + new_path, RED), false);
        ...
        if (!isValidPath(index_file, false))
            return (print_message("Invalid index file: " + index_file, RED), false);
        path = index_file;
    }
    else
        return (print_message("Invalid path: " + new_path, RED), false);
    return true;
}
