...
bool Config::validatelocation(int *i, Config::Location &tempLocation)
{
    if (tempLocation.getAllowMethods().empty() && tempLocation.getAutoindex().empty() &&
        tempLocation.getCgi().empty() && tempLocation.getIndex().empty() &&
        tempLocation.getReturn().empty() && tempLocation.getRoot().empty() &&
    ...
    {
        print_message("Error: Duplicate index files detected - each index file must be unique", RED);
        *i = 1;
        return false;
    }
    ...
    {
        print_message("Error: Root directory can only be specified once per location", RED);
        *i = 1;
        return false;
    }
